...
function corsHeaders(o: string) {
  const a = ALLOWED_ORIGINS.includes(o) ? o : ALLOWED_ORIGINS[0];
  return { 'Access-Control-Allow-Origin': a, 'Access-Control-Allow-Headers': 'authorization,x-client-info,apikey,content-type,x-veraluz-session,x-internal-secret', 'Access-Control-Allow-Methods': 'POST,OPTIONS' };
}
...
function detectIntent(msg: string, agentKey: string, last: string|null): string {
  ...
  if (agentKey==='finance_v1') {
    if (/revenu|chiffre|encaiss|paiement|finance/.test(m)) return 'finance_revenue';
  }
...
async function fetchData(admin: any, intent: string): Promise<{data:any,sources:string[]}> {
  const src: string[] = [];
  const sl = (a:any[]|null|undefined) => a!=null ? a.length : null;
  ...
  try {
    ...
    if (base==='contracts') {
      src.push('veraluz_contracts');
      return { data:{_intent:'contracts',note:'Table veraluz_contracts non disponible dans cette phase de construction.'},sources:src };
    }
    if (base==='restaurant') {
      const today=new Date().toISOString().split('T')[0];
...
function sv(v:any,fb='donnée indisponible') { return (v===null||v===undefined)?fb:String(v); }
...
function buildResp(agentKey:string, td:any, opMode:string): string {
  ...
  const name=N[agentKey]||agentKey;
  const sim=opMode==='construction_simulation'?'[SIM] ':'';
  const d=td.data;
  ...
  switch(d._intent) {
    ...
    case 'contracts': r+=d.note||`📄 Contrats : ${sv(d.active)} / ${sv(d.total)}\n`; break;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const {agent_key,message,conversation_id,session_token,history}=body;
    ...
    if (cid) { ctx=await loadCtx(admin,cid); }
    else {
      const {data:nc}=await admin.from('veraluz_agent_conversations').insert({agent_key,username:emp.employee_id,role:emp.role,title:message.slice(0,60),operational_mode:opMode}).select('id').single();
      cid=nc?.id||null;
    }
    ...
    if (cid) {
      await admin.from('veraluz_agent_messages').insert({conversation_id:cid,role:'user',content:message,operational_mode:opMode});
      await admin.from('veraluz_agent_messages').insert({conversation_id:cid,role:'assistant',content,tools_used:[intent],sources:td.sources.map((s:string)=>({table:s})),operational_mode:opMode});
      await admin.from('veraluz_agent_conversations').update({message_count:(history?.length||0)+2,last_message_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',cid);
    }
    await admin.from('veraluz_agent_usage_logs').insert({agent_key,conversation_id:cid,provider:'fallback',model:'keyword_intent_v4',input_tokens:Math.ceil(message.length/4),output_tokens:Math.ceil(content.length/4),latency_ms:Date.now()-t0,estimated_cost_usd:0,operational_mode:opMode});
    return new Response(JSON.stringify({conversation_id:cid,content,sources:td.sources,intent_detected:intent,follow_up_detected:isFollowUp(message)&&!!ctx.last_intent,llm_used:false,operational_mode:opMode,latency_ms:Date.now()-t0}),{status:200,headers:{...h,'Content-Type':'application/json'}});
  } catch(e:any) {
